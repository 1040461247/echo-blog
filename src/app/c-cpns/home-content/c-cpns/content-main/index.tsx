      ...
      {articlesData.map((item) => (
        <div className="article c-card mb-[38px]" key={item.id}>
          ...
          {item.album && (
            ...
            >
              <Image className="object-cover" src={item.album} fill alt="article-album" />
          ...
          <main className="flex flex-col gap-5 justify-between px-7 pt-5 pb-7">
            ...
            <div className="article-content text-left">
              <span className="ellipsis-2-line">{item.content}</span>
            </div>

            <footer className="article-info flex gap-3 text-sm text-gray-200/70">
              <div className="article-info-date">
                <i className="iconfont icon-archives mr-1" />
                <span>{item.createAt}</span>
              </div>

              <div className="article-info-category">
              ...
              <div className="article-read-more flex-1 text-right">
                <Link className="hover-highlight" href={`/articles/${item.id}`}>
                  Read more &gt;
                </Link>
              </div>

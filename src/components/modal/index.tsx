...
const Modal: FC<IProps> = memo(({ children, handleClose, title, subTitle }) => {
  ...
  return (
    <div className={`modal fixed inset-0 z-50`}>
      ...
      {/* Content */}
      <main
        className={`modal-content overflow-y-auto absolute inset-x-0 px-5 py-7 pt-[--ssm-modal-pt] mt-[50vh] -translate-y-1/2 mx-auto w-full h-full xs:w-[475px] xs:h-auto xs:p-6 rounded-2xl bg-[--bg-dark-blue] text-gray-300 shadow-lg transition-all duration-300 ${
          opened ? 'opacity-100 xs:!scale-100' : 'opacity-0 xs:scale-75'
        }`}
      >
        <header className="modal-content-header flex justify-between mb-5">
          <div className="moal-content-header-title flex flex-col flex-1">
